...
main() {
    ...
    while true; do
        for round in $(seq 1 "$repeat"); do
            ...
            current="$(date +%s)"
            current_hour="$(date +%H)"
            (( remaining_sec = 3600 - current % 3600 ))
            if [ "$fix_timezone" = "strict" ] && ( (( remaining_sec <= average + 15 )) || (( current_hour != start_hour )) ); then
                if (( remaining_sec < 120 )); then
                    log "Wait for event hour $start_hour to finish... (${remaining_sec}s)"
                    sleep "$remaining_sec"
                fi
                break
            fi
            ...
            stage_end="$(date +%s)"
            (( total_time = stage_end - start_time ))
            (( clear_count++ ))
            (( average = total_time / clear_count ))
            (( average_decimal = (total_time * 100 / clear_count) % 100 ))
            (( total_hours = total_time / 3600 ))
            (( total_minutes = (total_time % 3600) / 60 ))
            (( rate = clear_count * 3600 / total_time ))

            log "#%d Avg %d.%02ds, Rate %d/h, Total %dh%02dm" "$clear_count" "$average" "$average_decimal" "$rate" "${total_hours}" "${total_minutes}"
            #log "$round / $repeat"
            ...
            if [ "$exit_success" = "false" ]; then
                log "FAILED: The stage does not ended successfully. Waiting for manual correction."
