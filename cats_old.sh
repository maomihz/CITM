...
PAUSE_BUTTON_COLOR="ffcc00ff"
AUTO_DATE_TIME_ENABLED_COLOR="2093eeff"
...
RESET_STATE_INTERVAL=90  # Number of minutes to reset state
...
main() {
    ...
    while true; do
        for round in $(seq 1 "$repeat"); do
            ...
            if (( (stage_begin - start_time) / 60 / RESET_STATE_INTERVAL > state_reset_count )); then
                log "State reset needed immediately! (#$reset_state_count)"
                reset_state
                ((state_reset_count++))
                while [ "$(getcolor 44 88)" != "$PAUSE_BUTTON_COLOR" ]; do
                    log "Wait for stage to begin..."
            ...
            stage_end="$(date +%s)"
            ((clear_count++))
            average=$(( (stage_end - start_time) / clear_count ))
